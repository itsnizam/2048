...
public class MainView extends View {
    ...
    private void drawFeedbackButton(Canvas canvas) {
        drawDrawable(canvas, backgroundRectangle, sXFeedback, sYIcons, sXFeedback
                + iconSize, sYIcons + iconSize);
        drawDrawable(canvas, feedBackIcon, sXFeedback + iconPaddingSize, sYIcons
                + iconPaddingSize, sXFeedback + iconSize - iconPaddingSize, sYIcons
                + iconSize - iconPaddingSize);
    }

    private void drawShareButton(Canvas canvas){
        drawDrawable(canvas, backgroundRectangle, sXShare, sYIcons, sXShare
                + iconSize, sYIcons + iconSize);
        drawDrawable(canvas, shareIcon, sXShare + iconPaddingSize, sYIcons
                + iconPaddingSize, sXShare + iconSize - iconPaddingSize, sYIcons
                + iconSize - iconPaddingSize);
    }
    private void drawSoundButton(Canvas canvas) {

        Drawable iconToDraw = game.soundOn ? soundIcon : muteIcon;
        drawDrawable(canvas, backgroundRectangle, sXSound, sYIcons, sXSound
                + iconSize, sYIcons + iconSize);
        drawDrawable(canvas, iconToDraw, sXSound + iconPaddingSize, sYIcons
                        + iconPaddingSize, sXSound + iconSize - iconPaddingSize,
                sYIcons + iconSize - iconPaddingSize);
    }
    ...
    private void getLayout(int width, int height) {
        ...
        sXSound = (endingX - iconSize);
        sXUndo = sXSound - iconSize * 3 / 2 - iconPaddingSize;
        sXNewGame = sXUndo - iconSize * 3 / 2 - iconPaddingSize;
        sXLeaderboard = sXNewGame - iconSize * 3 / 2 - iconPaddingSize;
        sXFeedback = sXLeaderboard - iconSize * 3 / 2 - iconPaddingSize;
        sXShare = sXFeedback - iconSize * 3 / 2 - iconPaddingSize;
